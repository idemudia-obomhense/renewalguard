...
export function MonthBarChart({ byMonth, heroCurrency }: MonthBarChartProps) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-4 sm:p-6">
      ...
      <p className="mb-4 text-xs text-muted-foreground sm:mb-6 sm:text-sm">When active renewals are due this year</p>
          ...
          return (
            <div key={month} className="flex flex-1 flex-col items-center gap-1.5">
              <span className="text-[9px] text-muted-foreground sm:text-[11px]">
                {formatAmount(amount, heroCurrency)}
              </span>
              <div className="flex h-16 w-full items-end sm:h-24">
                <div
                  className={`w-full rounded-t-sm ${amount > 0 ? 'bg-primary' : 'border border-dashed border-border'}`}
                  style={{ height: `${heightPct}%` }}
                />
              </div>
              <span className="text-[9px] text-muted-foreground sm:text-[11px]">{MONTH_LABELS[month]}</span>
            </div>
          )
        })}
      </div>
